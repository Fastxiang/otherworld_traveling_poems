StartupEvents.registry('irons_spellbooks:spells', event => {
    ...
    event.create('fast:perfect_guard')
    .setCooldownSeconds(0)
    .setBaseManaCost(200)
    .setManaCostPerLevel(0)
    .setCastType('instant')
    ...
    .setUniqueInfo((spellLevel, caster) => { 
        let hp = 0;
        if (caster) {
            let CasterHp = caster.getAttribute('minecraft:generic.max_health').getValue();
            hp = CasterHp * 0.1
            if (hp > 100) hp = 100
            return [
            Component.green(Text.translatable(`item.irons_spellbooks.scroll.tooltip.heal.hp`, hp)), 
            Component.green(Text.translatable(`item.irons_spellbooks.scroll.tooltip.sword_technique_cd`, "持续期间+0.5秒"))
        ]
        } else {
        return [
            Component.black(Text.translatable(`item.irons_spellbooks.scroll.tooltip.heal.hp`, hp)), 
            Component.black(Text.translatable(`item.irons_spellbooks.scroll.tooltip.sword_technique_cd`, "持续期间+0.5秒"))
        ]
        }
    })
    .onCast(ctx => {
    ...
    .checkPreCastConditions(ctx => {
        return global.PerfectGuardCanUse(ctx)
    });
