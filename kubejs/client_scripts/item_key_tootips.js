...
ItemEvents.tooltip(event => {
    ...
    text.add(lineOffset++, `§8- §7${mobName}`);
          ...
          let bonusInfo = "";
          if (mob.EnderBonus) {
           bonusInfo = getEnderBonusNameList(mob.EnderBonus);
          }

          text.add(lineOffset++, `§8- §7${mobName}${bonusInfo} §8×${mob.count}`);
                        }
                    });
                    if (MobCount) {
...
function getEnderBonusNameList(bonusList) {
    if (!bonusList || bonusList.length === 0) return "";
    
    let names = bonusList.map(stack => {
        let ItemLang = stack.id.replace(':', '.')
                    let itemName = Text.translatable(`item.${ItemLang}`).getString();
                    if (itemName === `item.${ItemLang}`) {
                    itemName = Text.translatable(`block.${ItemLang}`).getString();
                    }
