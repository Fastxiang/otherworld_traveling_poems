...
registerItemMaidBauble("fast:lethal_shutter")
EnderBonusOnPlayerExposureModifyFrameData.addBonus("fast:lethal_shutter", LethalShutterEvent, 1)
EnderBonusOnPlayerExposureShutterOpening.addBonus("fast:lethal_shutter", LethalShutterCdEvent, 1)

function LethalShutterEvent(handler, event) {
    ...
    Cd /= 1 + CdSpeed
    addCooldown("lethal_shutter", uuid, Cd, level)
}

function LethalShutterCdEvent(handler, event) {
    let player = event.player;
    let level = player.level
    let uuid = player.uuid
    if (!isInCooldown("lethal_shutter", uuid, level)) return
    event.cancel()
}