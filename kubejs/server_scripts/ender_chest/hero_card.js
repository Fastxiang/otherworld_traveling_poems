...
let openGuiPlayer = {}
let openNewGuiPlayer = {}

ItemEvents.canPickUp(event => {
    let player = event.player
    if (!player || !player.isAlive()) return;
    let uuid = player.uuid
    if (openGuiPlayer[uuid]) {
    event.cancel();
    }
})
...
function OpenCardGuiEvent(player, Name, uuid, ItemList, originalItem) {
    ...
    player.openChestGUI(Text.translatable(Name), 1, gui => {
        ...
        gui.slot(0, 0, slot => {
            ...
            slot.setLeftClicked(() => {
                TheHeroCardClickedSlotEvent(slot, player, uuid, ItemList, originalItem, level, x, y, z, false)
                })
        });
        gui.slot(8, 0, slot => {
            slot.setItem(stickItem)
        ...
        for (let index = 0; index < ItemList.length; index++) {
            ...
            let Newitem = ItemList[index]
                ...
                slot.setLeftClicked(() => {
                TheHeroCardClickedSlotEvent(slot, player, uuid, ItemList, originalItem, level, x, y, z, true)
                })
            })
        }
    })
...
function TheHeroCardClickedSlotEvent(slot, player, uuid, ItemList, originalItem, level, x, y, z, SpawnEntity) {
