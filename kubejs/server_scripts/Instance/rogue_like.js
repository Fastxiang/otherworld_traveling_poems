...
function DeepblueCrystalEvent(player, item, PlayerPersistentData) {
			let open = false;
    ...
    if (ItemValue > 0) {
    openGUIgetRogueLikeEnderItem(player, ItemData.uuid, ItemData)
    }
    }
    return
    }
    ...
    RogueLikeEvent(player, item, PlayerPersistentData, 1)
    } else {
    openRogueStageSelectGUI(player, item, PlayerPersistentData)
    }
}

function openGUIgetRogueLikeEnderItem(player, targetUUID, useItemData) {
    let validItems = [];
...
function openRogueStageSelectGUI(player, item, PlayerPersistentData) {
    ...
    if (PlayerRogueData.stage) {
        ...
        player.openChestGUI(Text.of("§l请选择开始的层数"), 1, gui => {
            ...
            gui.slot(6, 0, slot => {
                ...
                slot.setLeftClicked(() => {
                openNewGuiPlayer[PlayerUuid] = true
                        RogueLikeEvent(player, item, PlayerPersistentData, PlayerRogueData.stage);
                });
            });
...
function RogueLikeEvent(player, itemWithNbt, PlayerPersistentData, Rostage) {
    ...
    gui.slot(slotIndex, 0, slot => {
        ...
        if (!isInsideMap || targetType == null) {
            ...
            slot.setLeftClicked(() => {
                ...
                let NextCount = 15
                if (hasEntityEnderBonus(player, "fast:deep_blue_core")) {
                NextCount = 1
                }
                if (dir === 'up') {
                dungeon.z -= NextCount;
...
function openMapGUI(player, mapData, PlayerRow, PlayerCol) {
    let PlayerUuid = player.uuid
    openNewGuiPlayer[PlayerUuid] = true
...
function RemoveRogueLike(level, uuid, PlayerUuid) {
    ...
    if (stage >= 10) {
     player.give(Item.of('fast:deep_blue_core'));
    }
    }
    }
    }
