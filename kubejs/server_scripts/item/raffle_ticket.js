...
ItemEvents.rightClicked(event => {
    ...
    if (nbt && nbt.RaffleTicket) ticketType = nbt.RaffleTicket;

    let itemList = global.ticketTypeObject[ticketType];
    ...
    // 判断是否批量抽奖（Shift+右键）
    let total = player.isShiftKeyDown() ? item.count : 1;

    if (total > 1) {
        player.tell(Text.of(`§6正在批量抽取 ${total} 张抽奖券...`).gold());
    }

    for (let t = 0; t < total; t++) {
        // 计算总权重
        let totalWeight = itemList.reduce((sum, it) => sum + it.weight, 0);
        let rand = Math.random() * totalWeight;
        let selectedItem = null;

        for (let i = 0; i < itemList.length; i++) {
            rand -= itemList[i].weight;
            if (rand <= 0) {
        ...
        if (selectedItem) {
            ...
            NewItemEntity(player.level, player.x, player.y, player.z, stack);
    ...
    if (player.isShiftKeyDown()) {
        player.tell(Text.of(`§a批量抽奖完成！共抽取 ${total} 张抽奖券！`).green());
    }
    }
    ...
    if (!player.isShiftKeyDown()) {
        ...
        OpenCardGuiEvent(player, "fast.ender.hero.card", NewUuid, itemList, item);
        return;
    }
...
'fast:the_card2': {r:{count:0.33, count2:0.33}, sr:{count:0.66, count2:0.66}, ssr:{count:0.99, count2:0.99}, name:'TheCard2', only: true},
'fast:the_card3': {r:{count:0.5, count2:0.7}, sr:{count:1, count2:0.5}, ssr:{count:2, count2:0.3}, name:'TheCard3', only: true},
